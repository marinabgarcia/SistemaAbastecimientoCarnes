...
<html>
    ...
    <body>
        ...
        <div class="col-lg-10 col-md-10" style=" margin:0px; padding:0px; background-color:rgba(252,252,252,1.00)">
            ...
            <div class="col-md-12" style="">
                <div class="table-responsive clearfix">
                    <table id="gridClientes" name="gridClientes" class="table table-hover table-striped" >
                        <thead>
                            <tr>
                                <th data-column-id="dniCliente" data-identifier="true" data-sortable="true" data-type="numeric">Dni</th>
                                ...
                                <th data-column-id="cuentaCorrienteCliente" data-formatter="precio" data-sortable="false">Cuenta Corriente</th>
                                ...
                                <th data-column-id="telefonoCliente" data-sortable="false">Telefono</th>
                                <th data-column-id="celularCliente" data-sortable="false">Celular</th>
                                <th data-column-id="emailCliente" data-sortable="false">Email</th>
                                <th data-column-id="dTipoCliente" data-formatter="tipo" data-sortable="false">Tipo</th>
                                        ...
                                        <table id="grid3" name="grid3" class="table table-striped">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th data-column-id="idVentaS" data-identifier="true" data-formatter="idCompra"  data-sortable="false">Id Venta</th>
        ...
        <script>
                    ...
                    function cargaPagos()
                    {
                        var grid2 = $("#gridClientes").bootgrid({
                            ajax: true,
                            rowCount: [40, 50, 60, 10],
                            url: "cargarTablaClientesB", formatters: {
                                "precio": function (column, row)
                                {
                                    return "<span class=\"label label-default\"><big>$ " + row.cuentaCorrienteCliente + "</big></span>";
                                },
                                "tipo"
                                        : function (column, row)
                                        {
                            ...
                            {
                                if ($(tipos.item(i)).attr("name") == "Carnicería")
                                {
                                    $(tipos.item(i)).addClass("label label-info");
                                }
                                if ($(tipos.item(i)).attr("name") == "Catering")
                                {
                                    $(tipos.item(i)).addClass("label label-success");
                                }
                                if ($(tipos.item(i)).attr("name") == "Gastronomía")
                                {
                                    $(tipos.item(i)).addClass("label label-warning");
                                }
                                if ($(tipos.item(i)).attr("name") == "Revendedor")
                                {
                                    $(tipos.item(i)).addClass("label label-primary");
                                }
                                if ($(tipos.item(i)).attr("name") != "Revendedor" && $(tipos.item(i)).attr("name") != "Gastronomía" && $(tipos.item(i)).attr("name") != "Catering" && $(tipos.item(i)).attr("name") != "Carnicería")
                                {
                        ...
                        }).on("click.rs.jquery.bootgrid", function (e, columns, row)
                        {
                            ...
                            if (seleccion != 0)
                            {
                                var datos = $('#gridClientes').bootgrid().data('.rs.jquery.bootgrid').currentRows;
                                var row;
                                for (var i = 0; i < datos.length; i++)
                                {
                                    if (datos[i].dniCliente == seleccion)
                                    {
                                        row = datos[i];
                                    }
                                }
                                //cargar info clientes
                                document.getElementById("dniClienteP").value = row.dniCliente;
                                document.getElementById("nombreClienteP").value = row.nombreCliente;
                                document.getElementById("apellidoClienteP").value = row.apellidoCliente;
                                document.getElementById("razonSocialClienteP").value = row.razonSocialCliente;
                                document.getElementById("idTipoClienteP").value = row.idTipoCliente;
                                document.getElementById("tipoClienteP").value = row.dTipoCliente;
                    ...
                    function cargaVentas()
                    {
                        ...
                        var grid3 = $("#grid3").bootgrid({
                            ...
                            {
                                return {
                                    dniCliente: document.getElementById("dniClienteP").value

                                };
                            },
                            
                            navigation: 0,
                            url: "cargaTablaVentasCliente",
                            formatters: {
                                "quantity"
                                        : function (column, row)
                                        {
                        ...
                        {
                            location.href = "venta.jsp?idVenta="+row.idVentaS;
